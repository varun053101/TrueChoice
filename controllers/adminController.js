  ...
  try {
    ...
    if (!title || !positionName || !startTime || !endTime) {
      return res.status(400).json({
        error: "title, positionName, startTime and endTime are required",
      });
    }
    ...
    return res
      .status(201)
      .json({ message: "Election created", election: saved });
  } catch (err) {
...
const deleteCandidate = async (req, res) => {
  try {
    ...
    if (voteCount > 0) {
      return res.status(400).json({
        error: `Cannot delete candidate: ${voteCount} vote(s) already recorded`,
      });
    }
...
const uploadEligibleVoters = async (req, res) => {
  try {
    const electionId = req.params.electionId;
