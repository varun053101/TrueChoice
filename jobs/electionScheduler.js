...
async function runElectionTimeUpdater() {
  ...
  try {
    ...
    const startResult = await Election.updateMany(
      startFilter,
      {
        $set: { status: 'ongoing' },
    ...
    if (startedAtResult) {
      ...
      const toStartDocs = await Election.find({ ...startFilter, startedAt: null }).select('_id startTime');
    ...
    const closeResult = await Election.updateMany(
      closeFilter,
      { $set: { status: 'closed' } }
    );
    ...
    if (closedAtResult) {
      ...
      const toCloseDocs = await Election.find({ ...closeFilter, closedAt: null }).select('_id endTime');
